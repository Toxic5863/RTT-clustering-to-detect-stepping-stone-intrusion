...
def get_time(stringx):
    ...
    return float(adjusted_time[0])*3600 + float(adjusted_time[1])*60 + float(adjusted_time[2]) + float(adjusted_time[3])
...
def get_source(stringx):
    return re.search(r'IP \d+\.\d+\.\d+\.\d+', stringx).group()[3:]

def get_destination(stringx):
    return re.search(r'>\s\d+\.\d+\.\d+\.\d+', stringx).group()[2:]
